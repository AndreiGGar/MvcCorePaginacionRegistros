...
namespace MvcCorePaginacionRegistros.Repositories
{
    ...
    /*CREATE VIEW V_DEPARTAMENTOS_INDIVIDUAL
        ...
        ///
        ALTER PROCEDURE [dbo].[SP_GRUPO_EMPLEADOS_OFICIO] (@POSICION INT, @CANTIDAD INT, @OFICIO NVARCHAR(50), @NUMEROREGISTROS INT OUT)
        AS
	        SELECT @NUMEROREGISTROS = COUNT(EMP_NO) FROM EMP WHERE OFICIO = @OFICIO
	        SELECT EMP_NO, APELLIDO, OFICIO, SALARIO, DEPT_NO FROM
	        (SELECT CAST(ROW_NUMBER() OVER (ORDER BY APELLIDO) AS INT) AS POSICION, EMP_NO, APELLIDO, OFICIO, SALARIO, DEPT_NO FROM EMP WHERE OFICIO = @OFICIO) AS QUERY
	        WHERE QUERY.POSICION >= @POSICION AND QUERY.POSICION < (@POSICION + @CANTIDAD)
     */
    #endregion
